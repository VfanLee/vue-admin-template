<template>
  <a-list :bordered="false">
    ...
    <template #footer>
      <a-space fill :size="0" :class="{ 'add-border-top': props.renderList.length < showMax }">
    ...
    <div v-if="props.renderList.length && props.renderList.length < 3" :style="{ height: (showMax - props.renderList.length) * 86 + 'px' }"></div>
  </a-list>
</template>
...
<script lang="ts" setup>
  ...
  const props = withDefaults(defineProps<Props>(), { unreadCount: 0 })
  interface Emits {
    itemClick: [MessageRecord[]]
  }
