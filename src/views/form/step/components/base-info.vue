<template>
  <a-form ref="formRef" :model="formData" class="form" :label-col-props="{ span: 6 }" :wrapper-col-props="{ span: 18 }">
    ...
    >
      <a-input v-model="formData.activityName" placeholder="输入汉字、字母或数字，最多20字符" />
    </a-form-item>
    <a-form-item
      field="channelType"
      label="渠道类型"
    ...
    <a-form-item
      ...
      :rules="[
        ...
        {
          type: 'url',
          message: '如 Android 或 iOS 的下载地址、中间跳转URL，网址必须以 http:// 或 https:// 开头',
        },
      ]"
      row-class="keep-margin"
    >
      <a-input v-model="formData.promoteLink" placeholder="请输入推广页面地址" />
      <template #help>
...
<script lang="ts" setup>
  ...
  interface Emits {
    changeStep: ['forward', BaseInfoModel]
  }
