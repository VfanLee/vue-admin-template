<template>
  <a-spin :loading="loading" style="width: 100%">
    <a-card :bordered="false" :style="props.cardStyle">
      <div class="content-wrap">
        <div class="content">
          <a-statistic :title="props.title" :value="renderData.count" :value-from="0" animation show-group-separator />
          <div class="desc">
    ...
    try {
      ...
      if (props.chartType === 'bar') {
        ...
        chartData.forEach((el) => {
          if (el.name === '2021') {
            if (lineData.value[0]) lineData.value[0].push(el.y)
          } else {
            if (lineData.value[1]) lineData.value[1].push(el.y)
          }
        })
