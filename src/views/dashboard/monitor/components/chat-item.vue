<template>
  <div :class="['chat-item', props.itemData?.isCollect ? 'chat-item-collected' : '']">
    <a-space :size="4" direction="vertical" fill>
      <a-typography-text type="warning">
        {{ props.itemData?.username }}
      </a-typography-text>
      <a-typography-text>{{ props.itemData?.content }}</a-typography-text>
...
<script lang="ts" setup>
  ...
  const props = withDefaults(defineProps<Props>(), { itemData: () => ({}) as ChatRecord })
</script>
